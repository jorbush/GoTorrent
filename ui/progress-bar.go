...
func (pb *PBar) RenderPBar(count float64, index int, numWorkers int) {
	if pb.winSize.Col == 0 || pb.winSize.Row == 0 {
		return // Not a terminal, running in a pipeline or test
	}
	...
	barWidth := int(math.Abs(float64(pb.wscol - pb.header - 25 - uint16(len(strconv.Itoa(index)))))) // Calculate the bar width
	...
	percentage := (count / float64(pb.Total)) * 100

	switch {
	case pb.wscol >= uint16(0) && pb.wscol <= uint16(9):
		fmt.Printf("[\x1B[33m%6.2f%%\x1B[0m]", percentage)
	case pb.wscol >= uint16(10) && pb.wscol <= uint16(20):
		fmt.Printf("[\x1B[33m%6.2f%%\x1B[0m] %s", percentage, bar)
	default:
		fmt.Printf("Downloaded piece #%d: [\x1B[33m%6.2f%%\x1B[0m] - Peers: %d %s", index, percentage, numWorkers, bar)
	}
}
